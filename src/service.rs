...
fn run_service() -> Result<()> {
    ...
    // Report a NON-ZERO exit code when we're dying because of an error.
    //
    // Windows only runs the configured failure actions (our auto-restart) when a service either
    // terminates without reporting Stopped, or reports Stopped with a non-zero code AND the
    // "restart on non-crash failures" flag is set. Reporting 0 on a fatal startup error — a port
    // already in use, an unreadable certificate — told the SCM "finished successfully", so the
    // service stayed dead and enforcement was silently off until a human noticed. `install` now
    // sets the flag; this supplies the code.
    let exit = match &result {
        Ok(()) => ServiceExitCode::Win32(0),
        Err(e) => {
            tracing::error!(error = %e, "service exiting with an error");
