...
pub const LIVE_AUDIT_WINDOW: std::time::Duration = std::time::Duration::from_secs(300);
...
/// # Why this exists
///
/// `screenshot_taken` used to be written once per frame, live frames included. At the live view's
/// cadence that is 1,200 lines an hour, 61 bytes each. `jsonl.rs` caps `audit.jsonl` at 2 MiB and
...
#[derive(Default)]
pub struct LiveViewAudit(std::sync::Mutex<LiveState>);
...
impl LiveViewAudit {
    ...
    pub fn observe(&self, now: std::time::Instant, window: std::time::Duration) -> Option<u64> {
...
mod tests {
    ...
    fn frames_inside_one_window_collapse_into_the_next_line() {
        let a = LiveViewAudit::default();
        let t0 = Instant::now();
        assert_eq!(a.observe(t0, W), Some(1));
        // A window's worth of live frames at the 3s cadence.
        for i in 1..100 {
            assert_eq!(
                a.observe(t0 + Duration::from_secs(i * 3), W),
                None,
    ...
    fn the_count_resets_only_when_a_line_is_actually_written() {
        ...
        let t0 = Instant::now();
        a.observe(t0, W);
        a.observe(t0 + Duration::from_secs(1), W);
        a.observe(t0 + Duration::from_secs(2), W);
        assert_eq!(a.observe(t0 + W, W), Some(3));
        a.observe(t0 + W + Duration::from_secs(1), W);
        assert_eq!(
            a.observe(t0 + W + W, W),
            Some(2),
    ...
    fn a_gap_longer_than_the_window_reports_again_at_once() {
        ...
        assert_eq!(a.observe(t0, W), Some(1));
        assert_eq!(
            a.observe(t0 + Duration::from_secs(3600), W),
            Some(1),
