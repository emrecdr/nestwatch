...
There are two exceptions, and both are worth stating precisely rather than hiding behind that sentence. The first: the
...
The second is yours to switch on, and off by default: under **Check from this PC** on the Integrations
card you can name a check program, which this PC then runs **as your child** on a schedule, with the
session your phone forwarded, to ask an integration what he has done today. That program contacts the
integration under his own account; the Nestwatch service itself still contacts nothing. It is
described in [`docs/PLUGIN-SYSTEM.md`](docs/PLUGIN-SYSTEM.md) and in the security model's outbound
section, and a household that names no program is exactly where the sentence above says.
...
| **Say something to him** | A box under the live view puts *your* words on his screen — "dinner in ten minutes" — and tells you whether it was shown or whether nobody is signed in at that PC. It arrives under a heading that names you rather than the system, so he can tell a person from a countdown. It is a nudge, not a note: it sits over whatever he is doing and clears itself after about half a minute. There is no reply — if you want a conversation, this is a poor substitute for one. |
...
| **Integrations** | Apps that earn screen time, which you install and switch off from the dashboard. A paired app reports only *that* its threshold was met — **how many minutes that earns is set here, on this PC, per integration** — so a report claiming 999 minutes grants exactly what you configured. An integration you switch off cannot grant at all, in one toggle, without unpairing anything. **Pair the right QR and a lost phone costs you very little.** `nestwatch pair --integration studygo` mints a credential that may add earned time *as that integration* and read today's total back — it cannot change what it earns, grant as you, watch your child's screen, or reach anything else here. The plain `nestwatch pair` QR is the one that signs a *person* in, and that one does hand a device everything you have, so keep it for people. Either way, a device you no longer control is signed out from its own row in **Signed-in devices**; you no longer have to change your password and re-pair the whole house. By default it is data you toggle, never code this machine runs or a server it reaches out to. The one exception is yours to switch on: name a check program under *Check from this PC* and this PC runs it as your child on a schedule, with the session your phone forwarded, and judges what it reports by the same rules. **He is told what the check found**, on his own screen and in the household's language: once a day when he is short, naming the nearest rung and what clearing it is worth, and *every* time practice buys him time. The asymmetry is deliberate — announcing every grant is what stops this being a thing that only ever says *not yet*, and rationing the shortfall notice to one a day is what stops a fifteen-minute timer becoming a nag. See [`docs/PLUGIN-SYSTEM.md`](docs/PLUGIN-SYSTEM.md). |
...
| **Refused today** | The limits don't only hold — you can see them holding. When the PC's clock is moved to shift the day boundary, when something tries to start the day over and wipe the tally, when a shutdown is cancelled with `shutdown /a`, or when a time code is submitted that buys nothing, Nestwatch declines it and now **says so on the dashboard**. Until this, every one of those refusals went to a log file that needs an Administrator console on the child's PC to read — the record existed exactly where you couldn't reach it. The card appears only when there is something to show, which is not most days, and it survives a reboot. It reports **what the tool did, not what anyone meant by it**: a family that really crossed a time zone produces the same count as a clock moved on purpose, so it says "clock change ignored — screen time and bedtime kept using the trusted time" and leaves the intent to you. That wording is why it's safe to show your child too. The time-code row follows the same rule: a child mistyping a code you read out over the phone counts exactly like one working through the possibilities, and the card says only that the code bought no time. |
| **Trust the setup** | `nestwatch doctor` reports whether the service is up, the port listening, the firewall rule right, the network private, the certificate valid, whether this Windows is new enough to take screenshots at all, whether the system drive is encrypted (without which the data directory can be read by booting another operating system), and whether anything is actually being enforced. Every problem prints its fix. |
| **Languages** | Two separate settings, because they answer to different people. **The child's surfaces** — their `/ask` page and every notice that reaches their desktop: the remaining-time countdowns, the bedtime warning, the lock warning, and the notice Windows shows as it powers the machine off — are set by the parent (`POST /api/language`) in **English, Dutch or Turkish**. Not detected from the browser: `Accept-Language` is set in the child's own browser, and the child does not get to choose the language of the notice telling them what is being watched. **The dashboard** has its own switch in the header, in the same three languages, kept in `localStorage` on the device you are reading it from — so two parents on two phones can differ, and neither changes a word of what the child is told. Both default to English. A language is all-or-nothing by construction: the Rust half will not compile until every child-facing message function answers for it, and two test guards discover the string tables rather than naming them, so a half-translated page fails instead of shipping. |
...
- The binary lives in `C:\Program Files\HostHealth\` and the config, cert, and logs in
  ...
  user can't read the password hash / TLS key / audit + usage logs, or delete the files *while
  Windows is running*. See the third hard limit below for where that stops.
...
- **The file permissions end where Windows does.** Every "locked to SYSTEM + Administrators"
  claim above describes Windows adjudicating access, which it can only do while it is the
  operating system running. Booting the PC from a USB stick reads the whole data directory
  directly — including the TLS private key, after which an impostor dashboard on your network
  can show the very fingerprint you were told to check. Encrypting the system drive is what
  closes this; `nestwatch doctor` now checks whether yours is and says so either way.
