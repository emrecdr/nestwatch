...
| **Curfew** | One or more time windows per weekday, **separate from the budget — and granting extra screen time does not move bedtime.** Counts down on the child's screen, then shuts down — and re-issues if the shutdown is cancelled. The dashboard says so at the moment it matters: approve a request while a window is open, or one long enough to run into the next one, and the confirmation tells you what will actually happen. |
