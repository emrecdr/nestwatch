...
- **Dependencies are gated on every push**, not at release time: `cargo deny check advisories
  sources licenses bans`, resolved against the shipped Windows target so the check can be
  blocking with no ignore list. The licence allow-list excludes GPL/AGPL/LGPL, which is what
  keeps the dependency tree compatible with the MIT licence this ships under.
